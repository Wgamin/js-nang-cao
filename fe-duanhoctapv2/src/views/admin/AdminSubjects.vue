...
const save = async () => {
  ...
  try {
    if (isEditing.value) {
      await fetchApi(`/admin/subjects/${form.value.id}`, { method: 'PUT', body: JSON.stringify(form.value) })
      showToast('Cập nhật thành công!')
    } else {
      await fetchApi('/admin/subjects', { method: 'POST', body: JSON.stringify(form.value) })
...
const destroy = async (item: any) => {
  ...
  try {
    await fetchApi(`/admin/subjects/${item.id}`, { method: 'DELETE' })
    showToast('Đã xoá!')
    loadData()
  } catch (e: any) { showToast(e.message, 'error') }
}
  ...
  <AppLayout>
    <div class="page">
      <div class="page-header">
        ...
        <button class="btn btn-primary" @click="openCreate">+ Thêm Môn</button>
      </div>

      <div class="card">
        <table class="table">
          <thead><tr><th>Tên Môn</th><th>Mô tả</th><th>Thao tác</th></tr></thead>
          <tbody>
            <tr v-for="item in data" :key="item.id">
              <td class="fw-bold">{{ item.name }}</td><td>{{ item.description || '—' }}</td>
              <td>
    ...
    <div v-if="showModal" class="modal-backdrop" @click.self="showModal=false">
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <label>Mô tả</label><input class="form-input" v-model="form.description" />
        </div>
